...
# -*- coding: utf-8 -*-
"""
Local testing version of test.py - Works on Windows without Raspberry Pi hardware
"""
...
# Try to import MediaPipe (use venv if available)
try:
    import mediapipe as mp
except ImportError:
    print("❌ MediaPipe not found. Using virtual environment...")
    # Try to use venv_mediapipe
    venv_python = os.path.join('venv_mediapipe', 'Scripts', 'python.exe')
    if os.path.exists(venv_python):
        print(f"⚠️  Please run: {venv_python} test_local.py")
        sys.exit(1)
    else:
        print("❌ Please install MediaPipe or use venv_mediapipe")
        sys.exit(1)

# Hardware imports - make optional for local testing
HAS_GPIO = False
HAS_PICAMERA = False
...
# --------------------------------------------------------------------
# CNN Model for image upload mode (optional)
# --------------------------------------------------------------------
MODEL_PATH = "fyp.h5"
...
# --------------------------------------------------------------------
# Hardware control functions (with fallback for local testing)
# --------------------------------------------------------------------
def move_servo(target_angle, step=10, delay=0.05):
    ...
    seq = range(int(current), int(target_angle) + (1 if target_angle > current else -1), 
                step if target_angle > current else -step)
    for angle in seq:
...
def ai_camera_mode():
    print("\n🤖 Starting MediaPipe AI Camera Mode (Press 'q' to exit)")
    print("⚠️  Hardware control is " + ("ENABLED" if HAS_GPIO else "SIMULATED (no hardware)"))
    
    cap = open_camera_with_fallbacks(preferred_index=0, width=640, height=480)
    if cap is None:
        print("❌ Could not open any camera. Close apps like Zoom/Teams and try again.")
        return
    ...
    try:
        while True:
            ...
            hw_status = "HARDWARE" if HAS_GPIO else "SIMULATED"
            cv.putText(dbg, f"Mode: {hw_status}",
                       (10, 110), cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2, cv.LINE_AA)
...
def manual_drive():
    ...
    print("⚠️  Hardware control is " + ("ENABLED" if HAS_GPIO else "SIMULATED"))
    while True:
...
def main_menu():
    ...
    # Auto-start mode 1 (MediaPipe AI Camera Drive) for quick testing
    # Uncomment the line below and comment out the input line to disable auto-start
    mode = "1"  # Auto-start MediaPipe AI Camera Drive
